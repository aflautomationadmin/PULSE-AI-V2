-- ============================================================
-- Procedure : kpi.GetSellThru
-- KPI        : Sell Thru %
--
-- Formula
--   SellThruPct = SUM(NETAMT) / NULLIF(SUM(SOH) + SUM(NETAMT), 0)
--
-- NOTE: This procedure requires an SOH column or equivalent stock-on-hand data.
--       If SOH is not available in prd.FACT_SALES_AI, this will need a view
--       or a different source table that contains stock levels.
--
-- Parameters
...
CREATE OR ALTER PROCEDURE kpi.GetSellThru
...
BEGIN
    ...
    SET @sel_cols +=
        ...
        SUM(SOH) AS TotalSOH,
        CASE
            WHEN SUM(SOH) + SUM(NETAMT) = 0 THEN 0
            ELSE SUM(NETAMT) / NULLIF(SUM(SOH) + SUM(NETAMT), 0)
        END AS SellThruPct
        ';
    ...
    + CASE
        ...
        ELSE ' ORDER BY SellThruPct DESC'
      END;
