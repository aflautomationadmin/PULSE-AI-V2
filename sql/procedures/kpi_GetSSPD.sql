-- ============================================================
-- Procedure : kpi.GetSSPD
-- KPI        : Sales Per Square Foot Per Day (SSPD)
--
-- Formula
--   SSPD = (SUM(NETAMT) / NULLIF(AVG(SIZE), 0)) / NULLIF(DISTINCTCOUNT(STORE_NAME), 0)
--   where INV_DAYS = distinct count of invoice dates
--
-- NOTE: this implementation uses DISTINCT COUNT of INV_DATE as INV_DAYS.
--       It assumes each invoice date contributes to the store-day denominator.
--
-- Parameters
...
CREATE OR ALTER PROCEDURE kpi.GetSSPD
...
BEGIN
    ...
    SET @sel_cols +=
        ...
        AVG(SIZE) AS AvgSize,
        COUNT(DISTINCT CAST(INV_DATE AS DATE)) AS INV_DAYS,
        COUNT(DISTINCT STORE_NAME) AS StoreCount,
        CASE
            WHEN AVG(SIZE) = 0 OR COUNT(DISTINCT STORE_NAME) = 0 THEN 0
            ELSE (SUM(NETAMT) / AVG(SIZE)) / COUNT(DISTINCT STORE_NAME)
        END AS SSPD
        ';
