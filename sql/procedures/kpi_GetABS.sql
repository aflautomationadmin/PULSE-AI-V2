-- ============================================================
-- Procedure : kpi.GetABS
-- KPI        : Average Basket Sales (ABS)
--
-- Formula
--   ABS = TotalQuantity / NetBills
--   TotalQuantity = SUM(QTY) for SALES invoices only
--   NetBills = COUNT(DISTINCT SALES invoices) - COUNT(DISTINCT non-SALES invoices)
--
-- Parameters
...
CREATE OR ALTER PROCEDURE kpi.GetABS
...
BEGIN
    ...
    SET @sel_cols +=
      ...
      - COUNT(DISTINCT CASE WHEN UPPER(INVOICETYPE) <> ''SALES'' THEN INV_CNT END)
        ...
        SUM(CASE WHEN UPPER(INVOICETYPE) = ''SALES'' THEN QTY ELSE 0 END)
            AS TotalQuantity,
